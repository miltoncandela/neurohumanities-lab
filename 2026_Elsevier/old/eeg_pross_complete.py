...
def calc_bands2(s, band, seconds):
    ...
    def calc_PSD(data):
        ...
        over = floor(DataFilter.get_nearest_power_of_two(sr)*19/20)
...
df = pd.read_csv('C:/Users/Milton/PycharmProjects/neurohumanities-lab/IEEE2026/pross/eeg.csv', index_col=0)
df_calib = df[df.Scene == 0].drop(['Second', 'Scene'], axis=1)
df = df[df.Scene != 0].drop(['Second'], axis=1)

for outlier_method in ['quantile', 'zscore']:
    for norm_method in ['minmax', 'zscore']:
        # pass
        norm_metric(df, df_calib, outlier_method, norm_method)
...
print(df_PSD)
df_PSD.reset_index(drop=True).to_csv('pross/PSD_TAB_calib.csv')